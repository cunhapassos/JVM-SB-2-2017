        ...
        for (int i = 0 ; i < var->valor.array_ref->length; i++) {
            switch (var->valor.array_ref->type) {
                case T_BOOLEAN:
                    ...
                    break;
                case T_CHAR:
                    wprintf(L"%c", (const wchar_t ) (*(int16_t*)var->valor.array_ref->area+sizeof(int16_t)*i));
                    break;
                case T_FLOAT:
                ...
                case T_DOUBLE:
                    ...
                    break;
                case T_BYTE:
                    printf("%d", *(int*)(var->valor.array_ref->area+sizeof(char)*i));
                    break;
                case T_SHORT:
...
//    printf(" %s %s\n", pMethodName->bytes, pMethodDescriptor->bytes);
    ...
    if (pClassFile == NULL) {
        ...
        if (pClassFile == NULL && (strcmp((char *) (pMethodName->bytes), "println" ) != 0)) {
        	return 2;
        } else if (pClassFile == NULL &&  (strcmp((char *) (pMethodName->bytes), "println" ) == 0)) {
...
void FU_ldc(ST_tpJVM *pJVM, ST_tpStackFrame *pFrame, u1 **PC){
    ...
    if (cpIndx->Integer.tag == CONSTANT_Integer ) {
        ...
        if(pString->StringObject == NULL){
            ...
            ST_tpObjectHeap *buf = VM_alocarObjetoDummie(nomeClasse);
            var2.valor.obj_ref = buf;

            VM_armazenarFieldDummie(var1, var2);
...
void FU_frem(ST_tpStackFrame *pFrame){
    ...
    var.valor.Float = fmodf(var2.valor.Float, var1.valor.Float);
    var.tipo = JFLOAT;
    PL_pushOperando(&pFrame->operandStack, var);
}
...
void FU_i2c(ST_tpStackFrame *pFrame){
    ST_tpVariable var1, var;
    var1 = *PL_popOperando(&pFrame->operandStack);
    var.valor.Char = (int16_t) var1.valor.Int;
    if (var.valor.Char < 0) {
