...
ST_tpObjectHeap *VM_alocarObjetoDummie(char *nomeClasse) {
    u1 *nome;
    int maxVariaveis;
    ST_tpObjectHeap *pObjeto;
    ST_tpClassFile *pAuxClassFile;
    ...
    pObjeto->className = malloc(sizeof(char) * strlen(nomeClasse) + 1);
    strcpy(pObjeto->className, nomeClasse);
    //printf("\n %s \n", pObjeto->className);
    pObjeto->thread = NULL;
    pObjeto->max_no_var = maxVariaveis;
    pObjeto->ref_count = 0;
...
void *VM_armazenarFieldDummie(ST_tpVariable var, ST_tpVariable objRef){
    ST_tpObjectHeap *pHeap;
    ...
    pHeap = (ST_tpObjectHeap *)objRef.valor.obj_ref;
    memcpy((pHeap->field_area + 0), &var, sizeof(var));
...
ST_tpJVM *VM_exucutarJVM(int numeroClasses, char *nomeClasses){
    ...
    */
	...
	//pMetodo = VM_procurarMetodo( pClasse, "()V", "<init>");
	pClasse = VM_carregarClasse(nomeClasses, pJVM);
